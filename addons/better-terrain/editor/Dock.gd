...
const SOURCE_BUTTON = preload("res://addons/better-terrain/editor/source_button.tscn")
...
@export var source_list: VBoxContainer
@export var label_warning_source_msg: Label

var is_dragging_source : bool
var cache_dragging_nodes : Array
var first_dragging_button_pressed : bool
...
func _ready() -> void:
    ...
    source_list.mouse_exited.connect(_on_source_list_mouse_exited)
    source_list.gui_input.connect(_on_source_list_gui_input)


func _process(delta):
    scroll_container.scroll_horizontal = 0

func _on_source_list_gui_input(event):
    if not is_dragging_source:
        return

    if event is InputEventMouseButton:
        if event.button_index == MOUSE_BUTTON_LEFT and event.is_released():
            is_dragging_source = false
            cache_dragging_nodes.clear()

func _on_source_list_mouse_exited():
    is_dragging_source = false
    cache_dragging_nodes.clear()
...
func tiles_changed() -> void:
    ...
    for idx in source_count:
        var source_id = tileset.get_source_id(idx)
        var source := tileset.get_source(source_id)
        if !(source is TileSetAtlasSource):
            continue

        var _name := source.resource_name
        if _name.is_empty():
            ...
            if !texture_name.is_empty():
                ...
                if !texture_path.is_empty():
                    _name = texture_path.get_file()

        if not _name.is_empty():
            _name += " "
        _name += " [ %d ]" % source_id

        var button : Button = SOURCE_BUTTON.instantiate()
        source_list.add_child(button)

        button.id = source_id
        button.text = _name
        button.button_pressed = source_id not in tile_view.disabled_sources
        button.pressed.connect(_on_terrain_enable_id_pressed.bind(source_id))
        button.gui_input.connect(_on_source_button_gui_input.bind(button))
        button.mouse_entered.connect(_on_source_button_mouse_entered.bind(button))

    if source_count == 0:
        label_warning_source_msg.visible = true
    else:
        label_warning_source_msg.visible = false
...
func _on_source_button_mouse_entered(button):
    if is_dragging_source and button not in cache_dragging_nodes:
        if button.button_pressed != first_dragging_button_pressed:
            button.button_pressed = !button.button_pressed
            cache_dragging_nodes.append(button)
            _on_terrain_enable_id_pressed(button.id)

func _on_source_button_gui_input(event, button):
    if event is InputEventMouseButton:
        ...
        if cache_dragging_nodes.size() == 0 and is_dragging_source:
            first_dragging_button_pressed = !button.button_pressed
            cache_dragging_nodes.append(button)
            _on_terrain_enable_id_pressed(button.id)
...
func add_terrain_entry(terrain:Dictionary, index:int = -1):
    ...
    entry.open_editor.connect(_on_edit_terrain_pressed)
...
func _on_terrain_enable_id_pressed(id):
    ...
    for button : Button in source_list.get_children():
        if button.button_pressed == false:
            disabled_sources.append(button.id)
